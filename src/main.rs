...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for card_num in numsvec {
        let mut res =
            reqwest::blocking::get(format!("https://arkhamdb.com/api/public/card/{card_num}"))?;
...
fn parse_asset(card: JsonValue, mut file: fs::File) {
    ...
    let code = &card["code"];
    let cost = &card["cost"];
    let skill_agility = if &card["skill_agility"] != &Null {
        &card["skill_agility"].to_string()
    } else {
        "0"
    };
    let skill_combat = if &card["skill_combat"] != &Null {
        &card["skill_combat"].to_string()
    } else {
        "0"
    };
    let skill_intellect = if &card["skill_intellect"] != &Null {
        &card["skill_intellect"].to_string()
    } else {
        "0"
    };
    let skill_willpower = if &card["skill_willpower"] != &Null {
        &card["skill_willpower"].to_string()
    } else {
        "0"
    };
    let skill_wildcard = if &card["skill_wildcard"] != &Null {
        &card["skill_wildcard"].to_string()
    } else {
        "0"
    };
    let xp = if &card["xp"] != &Null {
        &card["xp"].to_string()
    } else {
        "0"
    };
    ...
    let flavor = if &card["flavor"] != &Null {
        &card["flavor"].to_string()
    } else {
        ""
    };
    let subname = if &card["subname"] != &Null {
        &card["subname"].to_string()
    } else {
        ""
    };
    let restrictions = &if &card["restrictions"] != &Null {
        &card["restrictions"].to_string()
    } else {
        "{}"
    };
...
fn parse_enemy(card: JsonValue, mut file: fs::File) {
    ...
    let enemy_horror = if &card["enemy_horror"] != &Null {
        &card["enemy_horror"].to_string()
    } else {
        "0"
    };
    let enemy_fight = &card["enemy_fight"];
    let enemy_evade = &card["enemy_evade"];
    let victory = if &card["victory"] != &Null {
        &card["victory"].to_string()
    } else {
        "0"
    };
    let encounter_name = if &card["encounter_name"] != &Null {
        &card["encounter_name"].to_string()
    } else {
        ""
    };
    let encounter_position = if &card["encounter_position"] != &Null {
        &card["encounter_position"].to_string()
    } else {
        "0"
    };
    let pack_name = &card["pack_name"];
...
fn parse_location(card: JsonValue, mut file: fs::File) {
    ...
    let back_text = if &card["back_text"] != &Null {
        &card["back_text"].to_string()
    } else {
        ""
    };
    let back_flavor = if &card["back_flavor"] != &Null {
        &card["back_flavor"].to_string()
    } else {
        ""
    };
    let encounter_name = if &card["encounter_name"] != &Null {
